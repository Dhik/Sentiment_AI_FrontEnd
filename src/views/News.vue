    ...
    <div class="grid">
      <div class="col-6">
        <div class="card">
          ...
          <form @submit.prevent="fetchNews" class="formgroup-inline">
            <div class="field">
              <label for="query" class="p-sr-only">Enter Keyword</label>
              <InputText id="query" type="text" v-model="query" required />
            </div>
      ...
      <div class="col-3">
        <div class="card">
          <div class="flex justify-content-between mb-3" v-if="!loading && articles.length > 0">
            <div>
                <span class="block text-500 font-medium mb-3">{{ query }}</span>
                <div class="text-900 font-medium text-xl">{{ articles.length }} News</div>
            </div>
            <div class="flex align-items-center justify-content-center bg-cyan-100 border-round" style="width: 2.5rem; height: 2.5rem">
                <i class="pi pi-copy text-cyan-500 text-xl"></i>
            </div>
    ...
    <div class="col-6">
      <div class="card">
        ...
        <div v-else class="tweet-container">
          <div v-for="(article, index) in articles" :key="index" class="tweet">
            <div class="tweet-content">
              <div class="tweet-header">
                <img :src="article.source_icon" :alt="article.title" width="70" class="tweet-profile-pic">
                <div class="tweet-info">
                  <span class="tweet-author">{{ article.title }}</span><br>
                  <span class="tweet-author">{{ article.source_name }}</span><br>
                  <span class="tweet-author">{{ article.date }}</span><br>
                  <span class="tweet-price"><a :href="article.link" target="_blank">Read more</a></span>
                </div>
              </div>
...
removeWordCloud() {
    ...
    addItem() {
      if (this.newItem.trim() !== "") {
        ...
        this.articles = this.ori_articles;
        // this.filterArrays();
    ...
    deleteItem(index) {
      ...
      async fetchNews() {
        ...
        try {
          ...
          this.articles = data;
          this.ori_articles = data.comments;
